...
def saveResult(dic_path, Q, locations, total_acts):
    np.save(dic_path + '/SARSAoptimalQ.npy', Q)
...
def train():
    ...
    for i in range(N):
        ...
        while it < step_control:
            ...
            deta = reward + gamma * Q[new_greedy_act][indice_newp][indice_newv] - Q[greedy_act][indice_p][indice_v]
            Q[greedy_act][indice_p][indice_v] = Q[greedy_act][indice_p][indice_v] + lr * deta
            p, v, indice_p, indice_v, greedy_act = newp, newv, indice_newp, indice_newv, new_greedy_act
            locations.append(p)
            total_reward += reward
        total_step -= it
        if total_reward > optimal:
            optimal = total_reward
            saveResult('./result', Q, locations, total_acts)
        if (i+1) % 100 == 0:
            print("第%d-th | %d迭代平均代价为%.2f" % (i+1, N, total_step/100))
            total_step = 0
    show_result()
