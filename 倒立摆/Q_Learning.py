...
def saveResult(dic_path, Q, locations, total_acts):
    np.save(dic_path + '/QLoptimalQ.npy', Q)
...
def train(Numa, Numda):
    ...
    for i in range(N):
        ...
        while it < step_control and (np.abs((a-lowa) % (higha - lowa) + lowa) > controla or np.abs(da) > controlda):
            it += 1
            ...
            max_newQa = np.max(Q[:, indice_newa, indice_newda])
            reward = getR(a, da, actions[greedy_act])
            deta = reward + gamma * max_newQa - Q[greedy_act][indice_a][indice_da]
            Q[greedy_act][indice_a][indice_da] += lr * deta
            a, da, indice_a, indice_da = newa, newda, indice_newa, indice_newda
            locations.append(a)
            total_reward += reward
            error_a = np.abs((a-lowa) % (higha - lowa) + lowa)
        total_step += it
